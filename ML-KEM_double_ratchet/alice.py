...
# ====== Display Helper ======
def show_message(direction, count, kind, text, peer="Bob"):
    arrow = "→" if direction == "Send" else "←"
    print(f"[{direction} #{count}] {kind} {arrow} \"{text}\"")
    # reprint prompt after incoming messages so input isn't lost
    if direction == "Recv":
...
def encrypt_message(message_key, plaintext, aad):
    aesgcm = AESGCM(message_key)
    nonce = os.urandom(NONCE_LEN)
    ciphertext = aesgcm.encrypt(nonce, plaintext.encode(), aad)
    return nonce, ciphertext

def decrypt_message(message_key, nonce, ciphertext, aad):
    aesgcm = AESGCM(message_key)
...
async def handle_connection(reader, writer):
    ...
    # initial root -> send chain (empty shared secret)  (preserved)
...
async def handle_user_input(writer, state):
    while True:
        ...
        # Priority: if we have ek_b and our send_chain_key was reset, do cipher-only rekey against ek_b
        if state.ek_b is not None and state.send_chain_key is None:
            ...
            aad = CIPHER_IDENTIFIER  # AAD for cipher-only rekey
            nonce, ciphertext = encrypt_message(message_key, user_input, aad)
        ...
        # If after attempting above we still have no send chain, block (keeps original safety)
        if state.send_chain_key is None:
            print("(!) No send chain yet. Wait for Bob's first rekey message before sending.")
            continue
    ...
    while True:
        try:
            ...
            if data.startswith(CIPHER_IDENTIFIEREK):
                nonce = data[len(CIPHER_IDENTIFIEREK):len(CIPHER_IDENTIFIEREK) + NONCE_LEN]
                ciphertext = data[len(CIPHER_IDENTIFIEREK) + NONCE_LEN:]
                ...
                try:
                    ...
                    print(f"[!] KEM decapsulation failed on EK+Cipher-Rekey: {e}")
                    continue
                ...
                try:
                    plaintext = decrypt_message(message_key, nonce, ciphertext, CIPHER_IDENTIFIEREK)
                    state.recv_chain_key = next_chain
                    state.recv_count += 1
                    show_message("Recv", state.recv_count, "EK + Cipher Rekey", plaintext, peer="Bob")
                except Exception as e:
                    print(f"[!] Decryption failed on EK+CipherRekey: {e}")
                continue

            # --- Bob initiated cipher-only rekey (later rekeys) ---
            if data.startswith(CIPHER_IDENTIFIER):
                nonce = data[len(CIPHER_IDENTIFIER):len(CIPHER_IDENTIFIER) + NONCE_LEN]
                ciphertext = data[len(CIPHER_IDENTIFIER) + NONCE_LEN:]
                ...
                try:
                    ...
                    print(f"[!] KEM decapsulation failed on cipher-only rekey: {e}")
                    continue
                ...
                next_chain, message_key = derive_chain_and_message_key(state.recv_chain_key)
                try:
                    plaintext = decrypt_message(message_key, nonce, ciphertext, CIPHER_IDENTIFIER)
                    state.recv_chain_key = next_chain
                    state.recv_count += 1
                    show_message("Recv", state.recv_count, "Cipher-only Rekey", plaintext, peer="Bob")
                except Exception as e:
                    print(f"[!] Decryption failed on cipher-only rekey: {e}")
                continue
            ...
            nonce, ciphertext = data[:NONCE_LEN], data[12:]
            try:
                plaintext = decrypt_message(message_key, nonce, ciphertext, b"regular")
                state.recv_count += 1
                show_message("Recv", state.recv_count, "Regular", plaintext, peer="Bob")
            except Exception as e:
                # revert if failed
                state.recv_chain_key = recv_chain_key_before_step
                print(f"[!] Decryption failed on regular message: {e}")
            continue
