...
# ====== Display Helper ======
def show_message(direction, count, kind, text, peer="Alice"):
...
async def handle_user_input(writer, state):
    while True:
        ...
        if state.received_eka_flag and not state.first_rekey_done and state.send_chain_key is None:
            # encapsulate against Alice's ek_a
            ...
            show_message("Send", state.send_count, "EK + Cipher Rekey", user_input, peer="Alice")
            continue
        ...
        if state.first_rekey_done and state.send_chain_key is None:
            ...
            state.send_chain_key = next_chain
            state.send_count += 1
            show_message("Send", state.send_count, "Cipher-only Rekey", user_input, peer="Alice")
            continue
        ...
        state.send_chain_key = next_chain
        state.send_count += 1
        show_message("Send", state.send_count, "Regular", user_input, peer="Alice")
        ...
        try:
            ...
            if data.startswith(CIPHER_IDENTIFIER):
                ...
                try:
                    ...
                    state.recv_count += 1
                    show_message("Recv", state.recv_count, "Cipher-only Rekey", plaintext, peer="Alice")
                except Exception as e:
            ...
            try:
                ...
                state.recv_count += 1
                show_message("Recv", state.recv_count, "Regular", plaintext, peer="Alice")
            except Exception as e:
                state.recv_chain_key = prev_chain
                print(f"[!] Decryption failed on regular msg: {e}")
            continue
